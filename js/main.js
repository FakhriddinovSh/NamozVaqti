...
fetch('https://api.alquran.cloud/v1/quran/en.asad')
	...
	.then((data) => {
		...
		myData.forEach((item) => {
			...
			clonedTemplate.querySelector('.audio__number__desc').textContent =
				item.number;
			clonedTemplate.querySelector('.audio__name').textContent =
				item.englishName;
			clonedTemplate.querySelector('.audio__eng__name').textContent =
				item.englishNameTranslation;
			clonedTemplate.querySelector('.audio__ar-name').textContent =
				item.name;
			clonedTemplate.querySelector('.audio__ayahs').textContent =
				item.ayahs.length;
			clonedTemplate.querySelector('.audio__icon').dataset.id =
				item.number;
			clonedTemplate.querySelector(
				'.audio__icon-wrap',
			).href = `https://cdn.islamic.network/quran/audio-surah/128/ar.alafasy/${item.number}.mp3`;
